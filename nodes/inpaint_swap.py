"""NanoBanana Face Swap (Mask Inpaint) - Pathway C node.

The pathway that survives identity-recognition refusals: detect face,
mask the face polygon with FaceMesh, paint it solid red, send the
obscured image to Gemini. The model never sees the original face, so
the celebrity / public-figure classifier doesn't fire.
"""
...
class NanoBananaInpaintSwap:
        ...
        return {
            "required": {
                ...
                "mask_dilate_px": ("INT", {"default": 25, "min": 0, "max": 128,
                                            "tooltip": "Dilate the face polygon outward "
                                                       "before painting red. Higher = more "
                                                       "room for Gemini to redraw bone "
                                                       "structure. 25 is the tested default."}),
                "mask_feather_px": ("INT", {"default": 16, "min": 0, "max": 128,
                                             "tooltip": "Gaussian feather radius on the "
                                                        "final composite mask edge."}),
                "image_size": (IMAGE_SIZES, {"default": "2K"}),
                "color_match": ("BOOLEAN", {"default": True,
                    "tooltip": "LAB color match with face-polygon stats."}),
                "grain_match": ("BOOLEAN", {"default": True,
                    "tooltip": "Synthetic noise matched to target's cheek variance."}),
                "sharpness_match": ("BOOLEAN", {"default": True,
                    "tooltip": "Blur Gemini's face if it's sharper than the body."}),
                "composite_method": (COMPOSITE_METHODS, {"default": "laplacian"}),
                "match_directional_lighting": ("BOOLEAN", {"default": False,
                    "tooltip": "Same-character mode only. Corrupts identity in cross-identity swaps."}),
    ...
    def run(self, api_key, model, target_image, identity_1, scope, grid_mode,
            ...
            detector, crop_size, mask_dilate_px, mask_feather_px,
            image_size="2K",
        ...
        for i in range(iter_count):
            tgt, refs = iter_get(i)
            res = backend.swap_inpaint(
                target=tgt, refs=refs, scope=scope,
...
NODE_CLASS_MAPPINGS = {"NanoBananaInpaintSwap": NanoBananaInpaintSwap}
NODE_DISPLAY_NAME_MAPPINGS = {
    "NanoBananaInpaintSwap": "Nano Banana - Face Swap (Mask Inpaint)",
}
